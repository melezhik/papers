...
In few words sparrow acts _like_ cpan client for swat test suites. One may package swat tests into
_sparrow plugin_ and upload it into [SparrowHub](https://sparrowhub.org) - swat test suites repository.

This is what we need to do ...

Fisrt of all let's install sparrow:

    $ sudo cpanm Sparrow

Then get's [registered](https://sparrowhub.org/sign_up) at SPH and generate [API token](https://github.com/melezhik/sparrow#public-plugin)
...
And finaly upload a plugin for your swat tests suite:
...
It provides some meta information required for plugin successfully uploaded into SparrowHub.
...
Ok, we are almost set. A final step, need to declare CPAN modules dependencies if have any.
We are going to [cpanfile / carton](https://metacpan.org/pod/Carton) for this. A minimal dependency we have here is swat itself,
...
This is the end of Alex story. But now let's go back to John.


# It's monitoring script, John!

You saying merrily comming into his room. Now it's realy easy to setup monitpring for you!

All that John have to do is to grab his keyboards and type a few commands.


First of all, guess what? Yeah - cpanm Sparrow, as he needs it too!
...
Last two command just create a [sparrow project](https://github.com/melezhik/sparrow#create-a-project) for Alex application  and [checkpoint](https://github.com/melezhik/sparrow#create-checkpoints) called brand-new-web-service
to run monitoring checks againts it.


Then John probably need to install a plugin Alex just uploaded into SparrowHub. Now registration needed to
...
And finaly let's set credentials required for our tests:
...
# Let's cron does all the rest job 


    Now it's safe for John to put Alex appliction into production:
...
This will run monitoring checks every 10 mniutes and in case errors the swat test report will be emailed
to proper people.
