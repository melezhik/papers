...
to push your tests into production.
...
The first one, obviously is for letting user sign in and should be considered as most important part of the application.
...
And the second one, is not intended to hit by human - as it name hints - this is for getting some database information status
by ... right monitoring script, this endpoint was accidentally told by Alex as `GET  /database-info'
in his conversation with John. Upon hitting /database-status we will have:
...
And, yes , our application send cookies and make http redirect to /profile page upon successful login,
...
- Alex saying merrily coming into his room. "Now it's relay easy to setup monitoring you asked me!"
...
Last two command just create a [sparrow project](https://github.com/melezhik/sparrow#create-a-project) for Alex's application  and [checkpoint](https://github.com/melezhik/sparrow#create-checkpoints) called brand-new-web-service
to run monitoring checks against it.
...
And finally let's set credentials required for our tests:
...
# Let's cron does all the rest job
    ...
    Now it's safe for John to put Alex application into production:
